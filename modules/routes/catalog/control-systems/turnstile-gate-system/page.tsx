...
export default function TurnstileGateSystemPage() {
  ...
  return <div className={styles.page}>
    ...
    <Section id="access-control" tone="blue"><div className={styles.accessExperience}><div className={styles.integrationLayout}><div className={styles.integrationIntro}><p className={styles.eyebrow}>BEYOND THE GATE</p><h2 className={styles.integrationTitle}><span>Your entrance.</span><span>Your <mark>access rules.</mark></span></h2><p className={styles.integrationDescription}>Connect physical entry with the way your team verifies people, manages permissions and records attendance.</p><div className={styles.integrationNote}><Icon name="settings" /><span>Reader, controller and software compatibility determine the available features. Confirm the integration scope when choosing your model.</span></div><Link href="/contact/" className={`${styles.button} ${styles.integrationCta}`}>Discuss your existing system <Icon name="arrow" /></Link></div><div className={styles.accessGrid}>{accessMethods.map((method, index) => <article key={method.title}><div className={styles.accessCardTop}><span className={`${styles.iconBadge} ${styles.accessIcon}`}><Icon name={method.icon} /></span><span className={styles.accessNumber} aria-hidden="true">{String(index + 1).padStart(2, "0")}</span></div><h3>{method.title}</h3><p>{method.text}</p></article>)}</div></div><div className={styles.flowPanel}><div className={styles.flowHeader}><p className={styles.eyebrow}>HOW IT WORKS</p><h3>One connected entry workflow</h3></div><ol>{howItWorks.slice(0, 3).map((step, index) => <li key={step.title}><span>0{index + 1}</span><div><h4>{step.title}</h4><p>{step.desc}</p></div></li>)}</ol><p className={styles.flowFootnote}>Optional software workflows: attendance & HR reporting, visitor schedules and anti-passback rules, subject to the selected platform.</p></div></div></Section>
