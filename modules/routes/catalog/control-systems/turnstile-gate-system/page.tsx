...
function Section({ id, children, soft = false }: { id: string; children: ReactNode; soft?: boolean }) {
  return <section id={id} className={`${styles.section} ${soft ? styles.soft : ""}`}><div className={styles.container}>{children}</div></section>;
}
...
export default function TurnstileGateSystemPage() {
  ...
  return <div className={styles.page}>
    ...
    <Section id="turnstile-products" soft><div className={styles.catalogPanel}><Heading title="Browse Turnstile Products" text="Explore tripod turnstiles, flap barriers, swing gates and complete entrance solutions in one product explorer." /><TurnstileProducts items={turnstileCatalog} /></div></Section>

    <Section id="gate-types"><div className={styles.sectionHeader}><Heading eyebrow="FIND THE RIGHT FIT" title="Different entrances. The right gate." text="Sasha Corporation helps businesses choose the right gate for their entrance space, user volume, access method and budget." /><a href="#turnstile-products" className={styles.textLink}>Explore all models <Icon name="arrow" /></a></div><div className={styles.typeGrid}>{turnstileTypes.map((type, index) => {
    ...
    <Section id="gate-faq" soft><div className={styles.faqLayout}><div><Heading eyebrow="A FEW THINGS TO KNOW" title="Your questions, answered." text="Practical answers about gate selection, access methods, installation and support." /><Link href="/contact/" className={styles.textLink}>Talk through your requirements <Icon name="arrow" /></Link></div><div className={styles.faqList}>{faqs.map((item, index) => <details key={item.q}><summary><span>{String(index + 1).padStart(2, "0")}</span><h3>{item.q}</h3><span className={styles.faqToggle} aria-hidden="true" /></summary><p>{item.a}</p></details>)}</div></div><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }} /></Section>
