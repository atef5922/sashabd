...
export type TurnstileFeaturedCard = {
  product: TurnstileItem;
  features: string[];
  bestFor: string[];
};

export type TurnstileProductDetailPageProps = {
  product: TurnstileItem;
  ...
  featuredCards: TurnstileFeaturedCard[];
...
export default function TurnstileProductDetailPage({
  product,
  categoryLabel,
  categoryHref,
  backHref,
  ...
  recommendedFor,
  description,
}: TurnstileProductDetailPageProps) {
  const [activeTab, setActiveTab] = useState<"spec" | "description">("spec");

  const kindLabel = useMemo(() => getKindLabel(product.kind), [product.kind]);
  const recommended = useMemo(() => recommendedFor.slice(0, 4).join(", "), [recommendedFor]);
  const heroFeatures = useMemo(() => keyFeatures.slice(0, 10), [keyFeatures]);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6">
      ...
      <section
        className="grid gap-4 rounded-2xl border bg-white p-4 md:grid-cols-2"
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-700">Type: {kindLabel}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border bg-white p-4" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
            ...
            <ul className="mt-2 space-y-1 text-xs leading-6 text-slate-700">
              <li>- Lane planning and model recommendation</li>
              <li>- Wiring, controller integration and setup guidance</li>
              <li>- Software configuration and basic handover support</li>
              <li>- After-sales support and service-friendly commissioning</li>
            </ul>
          </div>
        </div>
        ...
        <section className="mt-6 rounded-2xl border bg-white p-4" style={{ borderColor: "rgba(15,23,42,0.1)" }}>
          <div className="hidden items-center justify-between gap-3 md:flex">
            <div>
              ...
              <p className="text-xs text-slate-600">Related items you may also compare for your setup.</p>
            </div>
          ...
          <div className="mt-4">
            <MobileFeaturedProductsRail
              ...
              items={featuredCards.map(({ product: item }) => ({
                id: item.slug,
                href: `${featuredHrefPrefix}/${item.slug}/`,
                title: item.title,
                imageSrc: item.image,
                imageAlt: item.title,
          ...
          <div className="mt-4 hidden gap-4 md:grid md:grid-cols-2 lg:grid-cols-3">
            {featuredCards.map(({ product: item, features, bestFor }) => {
              const detailHref = `${featuredHrefPrefix}/${item.slug}/`;
              const badge = getKindLabel(item.kind);
              ...
              return (
                ...
                >
                  <ProductGridCard
                    ...
                    topLeftBadge={{ text: badge, tone: "light" }}
